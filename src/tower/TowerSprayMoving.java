...
/**
 * A <code>TowerSprayMoving</code> object represents a Bleach tower icon drawn
 * to follow the mouse.
 * 
 ...
 */
public class TowerSprayMoving extends Tower
{
	private int price;
	private Color rangeColor;

	/**
	 * TowerSprayMoving constructor. Objects built of this class will have
	 ...
	 */
	public TowerSprayMoving(GameState state, int x, int y, int price)
	{
		super(state, x, y);
		this.price = price;
	...
	public void update(double timeElapsed)
	{
		...
		if (distanceToPath > 40)
		{
			...
			if (state.isMouseClicked())
			{
				...
				{
					state.addGameObject(new TowerSpray(state, position.x, position.y));
				}
		...
		{
			rangeColor = new Color(0.9f, 0.2f, 0.2f, 0.35f);
		}
	}
